            ...
            <form id="payment-form" method="POST" action="">
                <?php 
                    ...
                    // $uname=$_GET['uname'];
                        ...
                        $odate_formatted = date('Y-m-d H:i:s', strtotime($odate));
                        $uname_safe = mysqli_real_escape_string($conn, $uname);
                        $address_safe = mysqli_real_escape_string($conn, $address);
                        $q1 = mysqli_query($conn, "INSERT INTO order_history (user_id, user_name, address, zipcode, phone, email, total, order_date, status) VALUES ('$uid','$uname_safe','$address_safe','$zipcode','$phone','$email','$total','$odate_formatted',1)");


                        mysqli_query($conn,"update cart set status=1 where user_id=$uid");
