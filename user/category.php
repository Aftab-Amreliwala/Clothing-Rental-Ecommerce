...
if ($con && $cid > 0) {
    ...
    $subcategories = [];
    $sub_query = "SELECT * FROM subcategory_master WHERE cid = $cid AND status = 1 ORDER BY sid DESC";
    ...
    // Fetch products count for each subcategory
    $productCounts = [];
    foreach ($subcategories as $sub) {
        $prod_query = "SELECT COUNT(*) as count FROM product_master WHERE sid = " . $sub['sid'] . " AND status = 1";
        $prod_result = mysqli_query($con, $prod_query);
        if ($prod_result) {
            $prod_row = mysqli_fetch_assoc($prod_result);
            $productCounts[$sub['sid']] = $prod_row['count'];
        }
    }
}
...
<head>
    ...
    <title><?php echo $category ? htmlspecialchars($category['cname']) : 'Categories'; ?> - Elite Shoppy</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .subcat-card:hover .subcat-bg { transform: scale(1.1); }
        ...
        .subcat-card:hover .subcat-overlay { background: linear-gradient(to top, rgba(0,0,0,0.8) 0%, transparent 100%); }

        .subcat-card .subcat-content {
            position: absolute;
            bottom: 0;
            left: 0;
            right: 0;
        ...
        .subcat-card:hover .subcat-name { transform: translateY(-5px); }
...
<body>
    ...
    <div class="page-header">
        <div class="inner">
            <h1><?php echo $category ? htmlspecialchars($category['cname']) : 'Categories'; ?></h1>
            <div class="breadcrumb">
                <a href="Home.php">Home</a>
                <span>/</span>
                <span><?php echo $category ? htmlspecialchars($category['cname']) : 'Categories'; ?></span>
            </div>
        </div>
    </div>
        ...
        <div class="subcat-grid">
            <?php if (!empty($subcategories)): ?>
                <?php foreach ($subcategories as $i => $sub): ?>
                    <?php
                    $subImagePath = '../admin/images/' . htmlspecialchars($sub['photo']);
                    $hasSubImage = !empty($sub['photo']) && file_exists($subImagePath);
                    $bgImg = $hasSubImage ? $subImagePath : ($subFallbackImages[$i % count($subFallbackImages)]);
                    $prodCount = isset($productCounts[$sub['sid']]) ? $productCounts[$sub['sid']] : 0;
                    ?>
                    <a href="subcategory.php?cid=<?php echo $cid; ?>&sid=<?php echo $sub['sid']; ?>" class="subcat-card" style="text-decoration: none;">
                        <div class="subcat-bg" style="background-image: url('<?php echo $bgImg; ?>')"></div>
                        <div class="subcat-overlay"></div>
                        <div class="subcat-content">
                            <div class="subcat-name"><?php echo htmlspecialchars($sub['sname']); ?></div>
                            <div class="subcat-count"><?php echo $prodCount; ?> products available</div>
                        </div>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
    ...
    <script>
        ...
        const subcatCards = document.querySelectorAll('.subcat-card');

        const observer = new IntersectionObserver((entries) => {
            entries.forEach((entry, idx) => {
                if (entry.isIntersecting) {
                    const card = entry.target;
                    const index = Array.from(subcatCards).indexOf(card);
                    setTimeout(() => {
        ...
        subcatCards.forEach(card => observer.observe(card));
    </script>
</body>
</html>
