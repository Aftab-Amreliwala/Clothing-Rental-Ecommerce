...
if (isset($_GET["id"]) && !empty($_GET["id"]) && is_numeric($_GET["id"])) {
    ...
    $photo_query = mysqli_query($con, "SELECT photo FROM subcategory_master WHERE sid = $id");
    ...
    if ($photo_query && mysqli_num_rows($photo_query) > 0) {
        ...
        $q = mysqli_query($con, "DELETE FROM subcategory_master WHERE sid = $id");
        
        if ($q) {
            if (mysqli_affected_rows($con) > 0) {
                // Only delete the photo file if NO OTHER subcategory is using the same image
                if (!empty($photo_name)) {
                    // Check if any other subcategory is using the same photo
                    $check_photo = mysqli_query($con, "SELECT COUNT(*) as count FROM subcategory_master WHERE photo = '$photo_name'");
                    $check_result = mysqli_fetch_assoc($check_photo);
                    
                    // If no other subcategory is using this photo, then delete it
